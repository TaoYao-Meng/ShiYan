    ...
    public void run(){
        try {          
            ...
            while ((recvStr=in.readLine())!=null){ //只要客户机不关闭，则反复等待和接收客户机消息
               ...
               String news[]=recvStr.split(" ");
               //ServerUI.txtArea.append(toClientSocket.getRemoteSocketAddress()+ " 客户机用户姓名: "+clientCounts+" 消息："+recvStr+" ："+time+"\n"); //解析并显示收到的消息
               ServerUI.txtArea.append(toClientSocket.getRemoteSocketAddress()+ " 客户机用户姓名: "+news[0]+" 消息："+news[1]+" ："+time+"\n");

                //按照echo协议原封不动回送消息
               out.println(toClientSocket.getLocalSocketAddress()+ " 客户机用户姓名: "+news[0]+" Echo消息："+news[1]+" : "+time);
            }//end while   
